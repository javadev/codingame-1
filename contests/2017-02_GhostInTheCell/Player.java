...
class Player {
    ...
    private static List<Troop> allTroops; // replace with TroopRegistry?
    ...
    static void makeMove() {
        ...
        ///////////////////////////////////////////
        // ** VALID OUTPUT - ONE OUTPUT PER TURN **
        // Output.skipTurn();
        // Output.sendTroop(Factory sourceFactory, 
        //                  Factory targetFactory, 
        //                  int numberOfCyborgs) {
        // ** FOR DEBUGGING **
        // Output.LOG("something here");
    }
}
...
class InputParser {
    static FactoryMap getFactoryMap(Scanner in, int numberOfFactories) {
        ...
        for (int i = 0; i < numberOfLinks; i++) { // for each "road" between factories
            int factory1 = in.nextInt(); // factory ID
            int factory2 = in.nextInt(); // factory ID
            int distance = in.nextInt(); // number of turns to get from fac1 to fac2 (1<=n<=20)
    ...
    static List<Troop> getTroopsAndUpdateFactories(Scanner in, FactoryMap factoryMap) {
        ...
        int entityCount = in.nextInt(); // the total number of entities, both factories and troops
        for (int i = 0; i < entityCount; i++) {
            ...
            if (entityType.equals("FACTORY")) {
                ...
                Owner owner = Owner.get(arg1);
                int numberOfCyborgs = arg4; // n > 0
                int remainingTurnsToDestination = arg5; // n > 0
                allTroops.add(new Troop(owner, 
                                        factoryMap.getFactory(arg2), // source factory
                                        factoryMap.getFactory(arg3), // target factory
                                        numberOfCyborgs, 
